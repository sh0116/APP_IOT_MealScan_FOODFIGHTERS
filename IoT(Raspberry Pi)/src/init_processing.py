...
class Image_Processing:
	def __init__(self, image):
		...
		for self.box in np.array(self·side_dish):
			cv2.imwrite("/home/pi/osam/APP_IOT_MealScan_FOODFIGHTERS/IoT(Raspberry Pi)/asset/{}.png".format(dish_tag[self.cnt]), self.image[self.box[1][1]+5:self.box[0][1]-5,self.box[1][0]+5:self.box[2][0]-5].copy() )
			self.cnt+=1

		for self.box in np.array(self.main_dish):
			cv2.imwrite("/home/pi/osam/APP_IOT_MealScan_FOODFIGHTERS/IoT(Raspberry Pi)/asset/{}.png".format(dish_tag[self.cnt]), self.image[self.box[1][1]+10:self.box[0][1]-10,self.box[1][0]+10:self.box[2][0]-10].copy() )
			self.cnt+=1
